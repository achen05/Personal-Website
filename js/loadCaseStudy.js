...
function renderMediaSlideshow(media) {
    ...
    let thumbnails = media
        .map((item, index) => `
            <img class="thumbnail" src="${item.url}" alt="${item.alt || `Thumbnail ${index + 1}`}" data-index="${index}" loading="lazy">
        `)
        .join('');
...
function renderStoryMedia(item) {
    if (item.type === 'video') {
        ...
        return `
            <div class="storyMedia-container">
                <img class="modal-trigger" src="${item.url}" alt="${item.alt}" loading="lazy">
                <p>${item.caption}</p>
            </div>
        `;
    }
